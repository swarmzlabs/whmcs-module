...
- **Right-to-left localization for the client-area panel.** For a customer
  ...
  Urdu, …), the workspace panel now renders mirrored correctly — the
  balance figures, accent borders, and the "open editor" / "buy more"
  arrows follow the reading direction instead of staying left-to-right
  inside an RTL page. Left-to-right locales are unchanged.
